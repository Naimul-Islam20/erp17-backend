...
<div class="card" style="max-width:920px; margin:0 auto; padding:24px;">
    <div style="display:grid; gap:8px; padding-bottom:18px; border-bottom:1px solid #e5edf6;">
        <div style="font-size:30px; font-weight:700; line-height:1.2; color:#0f172a;">{{ $blog->title }}</div>
        <div style="display:flex; flex-wrap:wrap; gap:8px 16px; color:#64748b; font-size:14px;">
            <span>{{ $blog->category?->name ?? '-' }}</span>
            <span>{{ $blog->updated_at?->format('Y-m-d H:i') }}</span>
        </div>
    </div>
        ...
        @forelse ($blog->blocks as $block)
            @if ($block->type === 'image')
                <div style="display:flex; justify-content:center;">
                    <img src="{{ asset($block->image_path) }}" alt="Blog image" style="width:min(100%, 760px); display:block; border-radius:12px;">
                </div>
